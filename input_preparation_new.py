...
class DirectoryIterator(Iterator):
    def __init__(self, directory, image_data_generator,
        ...
        if label_name:
            label_file = h5py.File(label_name, "a")
            label_data = label_file.create_dataset("label", data=self.classes)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        batch_x_feature = np.zeros((len(index_array),) + (2014,), dtype=K.floatx())
        
        # build batch of image data
        for i, j in enumerate(index_array):
            fname = self.filenames[j]

            if self.out_type == 'feature':
                img_gray = cv2.imread(os.path.join(self.directory, fname), 0)
                img_feature = feature_vector(img_gray)
                assert img_feature.shape == (2014,), "detected feature length is uncorrect when processing get_batches_of_transformed_samples."
                batch_x_feature[i] = img_feature
                ...
                x = img_to_array(img_color, data_format=self.data_format)
        ...
        if self.save_to_dir:
            for i, j in enumerate(index_array):
                img = array_to_img(batch_x_color[i], self.data_format, scale=True)
                fname = '{prefix}_{index}_{hash}.{format}'.format(prefix=self.save_prefix,
                                                                  index=j,
        ...
        if self.class_mode == 'input':
            ...
            for i, label in enumerate(self.classes[index_array]):
                batch_y[i, label] = 1.
        elif self.out_type == 'feature':
            return batch_x_feature
        else:
